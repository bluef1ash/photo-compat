...
- `global.less` 精简并入 `tokens.less`,前端仅留 1 个 less 文件
...
1. **令牌架构**:`tokens.less` 重构(@import + @theme inline + :root/@media + @layer base 焦点环);删 `global.less`;`main.tsx` 单 import;验证 `vite build`
...
- 前端仅 `tokens.less` 一个 less 文件(global.less 删除)
